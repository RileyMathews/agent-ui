<script lang="ts">
	...
	onMount(() => {
		document.documentElement.classList.add('terminal-route');
		return () => document.documentElement.classList.remove('terminal-route');
	});
...
<style>
	...
	:global(html.terminal-route), :global(html.terminal-route body) { height: 100%; overflow: hidden; }
	:global(html.terminal-route body) { margin: 0; min-width: 20rem; background: #0b0d0e; color: #d8dfdd; font-family: Inter, ui-sans-serif, system-ui, sans-serif; }
